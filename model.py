...
def LowFusion(Z_l, Z_h, X):
    """Z = Z_l"""
    return Z_l


def HighFusion(Z_l, Z_h, X):
    """Z = Z_h"""
    return Z_h


def MeanFusion(Z_l, Z_h, X):
    """Z = 0.5 * Z_l + 0.5 * Z_h"""
    return 0.5 * (Z_l + Z_h)
...
class ConcatFusion(nn.Module):
    ...
    def forward(self, Z_l, Z_h, X):
        return torch.concat((Z_l, Z_h), dim=1)


class ScalarVectorFusion(nn.Module):
    """Z = c * Z_l + (1-c) * Z_h"""
    def __init__(self, in_dim, scalar=True):
        super(ScalarVectorFusion, self).__init__()
        if scalar:
            self.c = nn.Parameter(torch.tensor(0.))
        else:
            self.c = nn.Parameter(torch.zeros(size=(1, in_dim)))
...
class AttentionFusion(nn.Module):
    """Z = C[:,0] * Z_l + C[:,1] * Z_h, C is attention scores
     Reference: Chai, Ziwei, et al. "Can abnormality be detected by graph neural networks?." IJCAI. 2022."""
    def __init__(self, in_dim):
        super(AttentionFusion, self).__init__()
        self.W_f = nn.Sequential(nn.Linear(in_dim, in_dim),
                                 nn.Tanh(),)
        self.W_x = nn.Sequential(nn.Linear(in_dim, in_dim),
                                 nn.Tanh(),)

    def forward(self, Z_l, Z_h, X):
        Z_proj = self.W_f(torch.stack((Z_l, Z_h), dim=1))
        X_proj = self.W_x(X).unsqueeze(-1)
        C = nn.functional.softmax(torch.bmm(Z_proj, X_proj), dim=1)
        Z = Z_l * C[:, 0] +Z_h * C[:, 1]
        return Z


class NodeDimGatedFusion(nn.Module):
    def __init__(self, in_dim, emb_dim, num_gate_layers):
        super(NodeDimGatedFusion, self).__init__()
        assert num_gate_layers <= 2
        if num_gate_layers == 1:
            self.gate = nn.Linear(in_dim, emb_dim)
        else:
    ...
    def forward(self, Z_l, Z_h, X, return_coefs=False):
        C = torch.sigmoid(self.gate(X))
        Z = Z_l * C + Z_h * (1 - C)
        if return_coefs:
            return Z, C
        else:
...
class SAGAD(nn.Module):
    def __init__(self, in_dim, emb_dim, hid_dims, K, dropout, activation, mlp_norm, fusion):
        super(SAGAD, self).__init__()
        self.chebnet = ChebnetIIPropDGL(K)
        self.fusion = fusion
        if fusion.__class__.__name__ == 'ConcatFusion':
            self.mlp = MLP(2*emb_dim, hid_dims, dropout, activation, mlp_norm)
        else:
            self.mlp = MLP(emb_dim, hid_dims, dropout, activation, mlp_norm)
